<template>
    <div
        class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-6 font-sans"
    >
        <div class="relative w-full max-w-3xl max-h-[90vh] overflow-auto">
            <div
                class="absolute -inset-0.5 bg-linear-to-r from-cyan-500 via-blue-500 to-cyan-500 rounded-lg blur opacity-30"
            ></div>

            <div
                class="relative bg-linear-to-br from-[#0f1729] to-[#1a2235] rounded-lg shadow-2xl border border-cyan-500/20"
            >
                <div
                    class="flex items-center justify-between p-6 border-b border-cyan-500/20"
                >
                    <div>
                        <h2
                            class="text-2xl font-bold bg-linear-to-r from-cyan-300 to-blue-400 bg-clip-text text-transparent"
                        >
                            Ticket Details
                        </h2>
                        <p class="text-cyan-400 font-mono mt-1">
                            #{{ report.id }}
                        </p>
                    </div>
                    <button
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div
                            class="bg-[#0a0e1a]/50 rounded-lg p-4 border border-cyan-500/20"
                        >
                            <div
                                class="flex items-center gap-2 text-cyan-200/60 mb-2"
                            >
                                <MapPinIcon :size="16" />
  ...
  <p class="text-cyan-100 font-semibold">{{ report.submittedTime }}</p>
</div>
                    </div>
                    ...
                    >
                        <div
                            class="flex items-center gap-2 text-cyan-200/60 mb-3"
                        >
                    ...
                    <div class="border-t border-cyan-500/20 pt-6 space-y-4">
                        <h3 class="text-lg font-semibold text-cyan-100 mb-4">
                            Update Ticket
                        </h3>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-cyan-100 mb-3"
                                    >Urgency</label
                                >
                                <div class="space-y-2">
                                    <label
                ...
                <div class="flex gap-3 p-6 border-t border-cyan-500/20">
                    <button
                        @click="handleSave"
                        :disabled="isSaving"
                        class="flex-1 py-3 bg-linear-to-r from-cyan-500 to-blue-500 text-white rounded-lg hover:from-cyan-400 hover:to-blue-400 transition-all shadow-lg shadow-cyan-500/30 font-semibold disabled:opacity-50"
                    >
...
// 1. FIX: Use 'priority' because Home.vue renamed 'urgency' to 'priority'
...
const handleSave = async () => {
    ...
    try {
        const response = await axios.patch(
            `http://127.0.0.1:8000/api/admin/tickets/${props.report.id}`,
            {
                // 2. FIX: Use 'priority' to match your Laravel Controller's validation
