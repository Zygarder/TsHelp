<template>
        ...
        <div class="header-section">
          ...
          <div class="title-group">
            ...
            <div>
              ...
              <p class="subtitle">Submit a technical support ticket</p>
            </div>
          </div>
        </div>
            ...
            <form @submit.prevent="handleSubmit" class="issue-form">
              ...
              <div class="form-section">
                ...
                <div class="radio-group">
                  <label class="radio-card" :class="{ 'active': formData.issueType === 'hardware-computer' }">
                    <input 
                      type="radio" 
                    ...
                    />
                    <span class="radio-text">Hardware Malfunction - Computer Lab Unit</span>
                  </label>
                  
                  <label class="radio-card" :class="{ 'active': formData.issueType === 'appliance' }">
                    <input 
                      type="radio" 
                      v-model="formData.issueType" 
                      value="appliance" 
                      ...
                      @change="clearSpecificItem"
                    />
                    <span class="radio-text">Room Appliance (TVs, AC, etc.)</span>
                  </label>
                </div>
  
                <div v-if="formData.issueType" class="mt-4">
                  <label for="specificItem" class="input-label">
                    {{ formData.issueType === 'hardware-computer' ? 'Which PC?' : 'Which Appliance?' }} <span class="required-star">*</span>
                  </label>
                  <select id="specificItem" v-model="formData.specificItem" required class="custom-select">
                    <option value="" disabled>Select...</option>
                    
                    <template v-if="formData.issueType === 'hardware-computer'">
                      <option v-for="n in 20" :key="n" :value="'PC-' + n.toString().padStart(2, '0')">
                        PC-{{ n.toString().padStart(2, '0') }}
                      </option>
                    </template>
              ...
              <div class="form-section">
                ...
                <select id="location" v-model="formData.location" required class="custom-select">
                  ...
                  <optgroup label="Offices">
                    ...
                    <option value="CEIT-Dean">CEIT Dean Office</option>
                  </optgroup>
                </select>
              </div>
              ...
              <div class="form-section no-border">
                <label for="description" class="section-label">
                  Please describe the issue in detail <span class="required-star">*</span>
                </label>
                <p class="helper-text">
                  Include any relevant information such as error messages, when the issue started, and what you were doing when it occurred.
                </p>
                <textarea 
                  id="description" 
  ...
  const handleSubmit = () => {
    // We'll hook this up to Laravel later!
    console.log('Submitting ticket:', formData);
    
    // For now, redirect to a success page or back to dashboard
    router.push({ 
    path: '/success', 
    state: { type: 'issue' } 
  }); 
  ...
  .red-icon { background: linear-gradient(to bottom right, #ef4444, #f97316); box-shadow: 0 10px 15px -3px rgba(239, 68, 68, 0.5); }
  ...
  .issue-form { display: flex; flex-direction: column; gap: 2rem; }
  ...
  .radio-card:hover { border-color: rgba(34, 211, 238, 0.5); }
  .radio-card.active { border-color: #22d3ee; background-color: rgba(34, 211, 238, 0.05); }
  ...
  .radio-card:hover .radio-text { color: #67e8f9; }
  
  /* --- Inputs & Selects --- */
  .mt-4 { margin-top: 1rem; }
  .input-label { display: block; color: #cffafe; margin-bottom: 0.75rem; font-size: 0.875rem; }
  
  .custom-select, .form-textarea {
    width: 100%;
  ...
  .custom-select option, .custom-select optgroup { background-color: #0f1729; color: #cffafe; }
  
  .form-textarea { resize: vertical; min-height: 120px; }
  .form-textarea::placeholder { color: rgba(6, 182, 212, 0.3); }
  .custom-select:focus, .form-textarea:focus { outline: none; border-color: #22d3ee; box-shadow: 0 0 0 2px rgba(34, 211, 238, 0.2); }
  
  .helper-text { font-size: 0.875rem; color: rgba(165, 243, 252, 0.6); margin: 0 0 0.75rem 0; line-height: 1.4; }
