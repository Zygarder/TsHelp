...
// 2. Make this function async so we can "await" the database response
const handleLogin = async () => {
    ...
    // 3. The REAL API Call (Replace the setTimeout with this)
    try {
        // This is where the 'response' variable is officially born!
        const response = await axios.post('http://127.0.0.1:8000/api/login', {
            email: email.value,
            password: password.value
        });

        // The Traffic Cop: Check the role sent back from Laravel
        ...
        // If they type the wrong password, Laravel sends a 401 Unauthorized error
...
const buttonText = computed(() =>
    isLoading.value ? "Authenticating..." : "Sign In"
);
</script>
...
.submit-button:hover:not(:disabled) {
    ...
    box-shadow: 0 10px 15px -3px rgba(34, 211, 238, 0.4),
        0 4px 6px -2px rgba(34, 211, 238, 0.4);
}
